<template>
  <main class="product-line-page pb-152 pb-80">
    <SectionCoverSwitch
...
export default {
  ...
  nuxtI18n: {
    paths: {
      it: '/prodotti/linee/:slug/',
      en: '/products/lines/:slug/',
    },
  },
  extends: BasePage,
  async asyncData({ app, store, params }) {
    ...
    const { productLine } = await ApiService.getProductLinePage(
      locale,
      params.slug
    )
    const slugLocales = productLine._allSlugLocales
    ...
    const { allProducts } = await ApiService.getLineProducts(
      locale,
      productLine.id
    )
    ...
    return {
      sectionCoverSwitch: {
        ...
        description: productLine.shortDescription,
        img: handleAltText(productLine.image),
        links: allProductLines.map((line) => ({
          label: line.name,
          url: handleSlug(locale, 'line', line.slug),
        })),
      },
