<template>
  <main class="product-aging-page pb-152 pb-80">
    <SectionCoverSwitch
...
export default {
  ...
  nuxtI18n: {
    paths: {
      it: '/prodotti/stagionatura/:slug/',
      en: '/products/aging/:slug/',
    },
  },
  extends: BasePage,
  async asyncData({ app, store, params }) {
    ...
    const { productAging } = await ApiService.getProductAgingPage(
      locale,
      params.slug
    )
    const slugLocales = productAging._allSlugLocales
    ...
    const { allProducts } = await ApiService.getAgingProducts(
      locale,
      productAging.id
    )
    ...
    return {
      sectionCoverSwitch: {
        title: productAging.name,
        description: productAging.shortDescription,
        img: productAging.image ? handleAltText(productAging.image) : '',
        links: allProductAgings.map((aging) => ({
          label: aging.name,
          url: handleSlug(locale, 'aging', aging.slug),
        })),
      },
      ...
      seoText: productAging.seoText,
      seo: productAging.seo,
    }
  },
  data() {
    return {
